...
const colors = ['bg-red-500', 'bg-orange-500', 'bg-yellow-500', 'bg-green-500', 'bg-teal-500', 'bg-cyan-500', 'bg-violet-500', 'bg-blue-500']
...
<template>
  <Transition name="modal">
    <div v-if="show" id="overlay" @click="$emit('closeModal')" @keyup.esc="$emit('closeModal')" class="modal-mask absolute top-0 left-0 flex w-full h-full bg-neutral-900/70 z-10">
      <div @click.stop="" class="modal-container flex flex-col relative m-auto w-[48rem] h-96 bg-white border-none rounded-xl p-7">
        <div class="flex items-center justify-between w-1/2 m-auto">
          <label v-for="color, index in colors" :key="index" class="w-6 h-6 rounded-full">
            <input type="radio" :value="index" v-model="select" :class="color" class="appearance-none transition-all w-full h-full rounded-full cursor-pointer checked:border-4 checked:border-zinc-600">
          </label>
        </div>
        <textarea ref="textRef" v-model="text" @keyup.ctrl.enter="emitAddNote" class="h-full p-4 mt-3 border-2 rounded-xl resize-none" autofocus></textarea>
...
.btn {
  @apply p-3 text-2xl rounded-xl text-white w-1/2 mx-3;
}
